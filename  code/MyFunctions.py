...
def callYolo():
	...
	r_v = os.system(main) 
	main = "darknet.exe detect yolov3-voc.cfg .\\results_mine\\yolov3-voc_last.weights data\\streetview\\local_image2.jpg"
	r_v = os.system(main) 
	main = "darknet.exe detect yolov3-voc.cfg .\\results_mine\\yolov3-voc_last.weights data\\streetview\\local_image3.jpg"
	r_v = os.system(main) 
	print (r_v )
...
def setMap(location):
	...
	y = str(location[1])
	key = 
	image_ur = "http://maps.googleapis.com/maps/api/staticmap?center="+x+","+y+"&zoom=15&size=300x200&maptype=roadmap&markers=color:red%7C"+x+","+y+"&key="+key
	resp = requests.get(image_ur, stream=True)
	local_file = open('data/streetview/local_image.png', 'wb')
...
def getImg(location):
	...
	y = str(location[1])
	key = 
	image_url0  = "https://maps.googleapis.com/maps/api/streetview?size=2048x2048&fov=75&pitch=15&location="+x+","+y+"&heading=45&key=" + key
	image_url90 = "https://maps.googleapis.com/maps/api/streetview?size=2048x2048&fov=75&pitch=15&location="+x+","+y+"&heading=135&key=" + key
	image_url180= "https://maps.googleapis.com/maps/api/streetview?size=2048x2048&fov=75&pitch=15&location="+x+","+y+"&heading=225&key=" + key
	image_url270= "https://maps.googleapis.com/maps/api/streetview?size=2048x2048&fov=75&pitch=15&location="+x+","+y+"&heading=315&key=" + key
